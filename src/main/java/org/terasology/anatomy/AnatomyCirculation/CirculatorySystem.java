...
public class CirculatorySystem extends BaseComponentSystem {
    ...
    private final float MINOR_BLEEDING_THRESHOLD = 0.8f;
    private final float BLEEDING_THRESHOLD = 0.5f;
    private final float SEVERE_BLEEDING_THRESHOLD = 0.2f;
    ...
    public void onGather(AnatomyStatusGatheringEvent event, EntityRef entityRef,
        ...
        if (event.getSystemFilter().equals("") || event.getSystemFilter().equals("Skeletal")) {
            for (Map.Entry<String, List<String>> injuredCirculatoryEffect :
                    injuredCirculatoryComponent.parts.entrySet()) {
